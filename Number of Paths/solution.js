...
function findPath(x, y) {
    if (y > x || x < 0 || y < 0) return 0             // off the grid or crossed line
    else if (x === 0 && y === 0) return 1             // valid path
...
function findPaths(x, y, memo) {
    if (x < 0 || y < 0 || y > x) memo[x][y] = 0        // respect boundaries
    else if (memo[x][y] !== -1) return memo[x][y]      // if not -1, we have already calculated this value
    else if (x === 0 && y === 0) memo[x][y] = 1        // reach destination
